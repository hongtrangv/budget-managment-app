...
def get_shelves_layout():
    ...
    It reconstructs the layout into the nested structure required by the frontend.
    """
    try:
        # This query requires a composite index on 'row' (asc) and 'order' (asc)
        ...
        # Provide a default layout if the 'shelves' collection is empty to prevent UI from breaking
        if not layout:
             layout = [
        ...
        # Return a generic error to the client, while logging the specific error on the server
        print(f"ERROR in /api/shelves: {e}") # Keep this for server-side logging
        return jsonify({"error": "Failed to fetch shelf layout."}), 500


# --- Book API Endpoints (Standardized) ---
